...
# Maximum amount that can be dragged
...
func scale_arrow() -> void:
	var impulse_lenght = get_impulse().length()
	var scale_percentage = impulse_lenght / IMPULSE_MAX
	
	arrow.scale.x = (_arrow_scale_x * scale_percentage) + _arrow_scale_x
...
func _on_sleeping_state_changed():
	if sleeping:
		...
		# this takes the colliding cup and kills it
		if(cb.size() > 0):
			cb[0].die()
		call_deferred("die")
