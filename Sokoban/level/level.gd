...
func cell_is_empty(cell: Vector2i) -> bool:
	return not cell_is_wall(cell) && not cell_is_box(cell)
...
func get_atlas_coord_for_layer_name(layer_name: String) -> Vector2i:
	match layer_name:
		LAYER_KEY_FLOOR:
			# Takes one random floor tile (3 to 8 in the Atlas)
...
func add_tile(tile_coord: Dictionary, layer_name: String) -> void:
	...
	var atlas_vector = get_atlas_coord_for_layer_name(layer_name)
...
func add_layer_tiles(layer_tiles, layer_name: String) -> void:
	for tile_coordinate in layer_tiles:
		add_tile(tile_coordinate.coord, layer_name)
